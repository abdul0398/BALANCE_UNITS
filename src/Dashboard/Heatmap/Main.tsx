...
const Treemap: React.FC<TreemapProps> = ({ data, width, height }) => {
  ...
  useEffect(() => {
    ...
    const root = d3.hierarchy<DataNode>(data).sum((d) => d.value);
    ...
    svg
      .selectAll<SVGRectElement, TreeNode>("rect")
      ...
      .on("mouseover", function (event, d) {
        ...
        setTooltip({
          ...
          text: `Project: ${d.data.name}
            \n, Balance Units: ${
              projectRelation[d.data.name][
                projectRelation[d.data.name].length - 1
              ].unitsAvail -
              projectRelation[d.data.name][
                projectRelation[d.data.name].length - 1
              ].soldToDate
    ...
    svg
      .selectAll<SVGTextElement, TreeNode>("text")
      .data(root.leaves() as TreeNode[])
      .enter()
      .append("text")
      ...
      .text((d) =>
        d.data.name.length > 10 ? d.data.name.slice(0, 10) + "..." : d.data.name
      )
      .attr("fill", "white")
      .style("font-size", "8px");
  }, [data, width, height]);
