...
export default function TableDistricts() {
    ...
    if (selectedDistricts.length > 0) {
        filteredData = data.filter((item) => selectedDistricts.includes(item.district));
    }

    if (selectedProjects.length > 0) {
        filteredData = filteredData.filter((item) => selectedProjects.includes(item.project));
    }

    if (selectedType !== "ALL") {
        filteredData = filteredData.filter((item) => item.marketSegment === selectedType);
    }
    ...
    const districtData = districtArray.map((district) => {
        ...
        const unitSoldInDistrict = projectsInDistrict.reduce((acc, project: any) => {
            const projectData = projects[project];
            if (!projectData) return acc;
            if (projectData.listing.unitsAvail - projectData.listing.soldToDate != 0) {
                return acc + projectData.listing.soldToDate;
            };
            return acc;
        }, 0);

        const totalUnitsInDistrict = projectsInDistrict.reduce((acc, project: any) => {
            const projectData = projects[project];
            if (!projectData) return acc;
            if (projectData.listing.unitsAvail - projectData.listing.soldToDate != 0) {
        ...
        return {
            [district]: {
                ...
                marketSegment: ((unitSoldInDistrict / totalUnitsInDistrict) * 100).toFixed(2) + "%"
            }
        }
                ...
                <tbody className="text-xs">
                    {
                        districtData.map((district, index) => {
                            ...
                            if (districtData.listing.soldToDate === 0 && districtData.listing.unitsAvail === 0) return null;
                            return (
                                <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td className="px-6 py-4">
                                        {districtData.marketSegment}
                                    </td>
                                    <td className="px-6 py-4">
                                        {districtData.listing.unitsAvail - districtData.listing.soldToDate}
                                    </td>
                                </tr>
                            );
                        })
